    ...
    let rec go (tm : term) (motive : term) (st : proof_state) : proof_state * term =
      ...
      | Name "Measure" -> (
          ...
          | Some m -> (
              ...
              let proof = mk_app (mk_app (mk_app (mk_app (mk_app (mk_app (mk_name "Eq.elim") (mk_name "Measure")) tm_normal) motive_fun) goal_hole) tm) tm_normal_eq_tm in
      ...
      | _ -> (
          ...
          match tm.inner with
          | App (f, arg) ->
            let left_motive = subst motive (Bvar motive_bid) (App (mk_bvar motive_bid, arg)) in
            let st, f' = go f left_motive st in
            let right_motive = subst motive (Bvar motive_bid) (App (f', mk_bvar motive_bid)) in
