(* For constructing elaborator-level terms in tests *)

open Elab.Term

let l = dummy_range
let name s = { inner = Name s; loc = l }
let bvar n = { inner = Bvar n; loc = l }
let hole n = { inner = Hole n; loc = l }
let app t1 t2 = { inner = App (t1, t2); loc = l }

(* "named" fun, fun with named argument *)
let nfun arg_name bid ty body = { inner = Fun (Some arg_name, bid, ty, body); loc = l }

(* "unnamed" fun, fun with no argument name *)
let ufun bid ty body = { inner = Fun (None, bid, ty, body); loc = l }
let narrow arg_name bid ty ret = { inner = Arrow (Some arg_name, bid, ty, ret); loc = l }
let uarrow bid ty ret = { inner = Arrow (None, bid, ty, ret); loc = l }
let sort n = { inner = Sort n; loc = l }
