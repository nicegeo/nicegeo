...
let test_const_lookup () =
  ...
  Alcotest.check' Testable.term ~msg:"Point is of Sort 1" ~actual:ty
    ~expected:(Sort 1);

  let ty_line = try_infer env (Hashtbl.create 0) (Const "Line") in
  Alcotest.check' Testable.term ~msg:"Line is of Sort 1" ~actual:ty_line
    ~expected:(Sort 1)
...
let test_fvar_lookup () =
  ...
  Alcotest.check' Testable.term ~msg:"p1 is of type Point" ~actual:ty
    ~expected:(Const "Point")

let test_fvar_unknown_fails () =
  let env = mk_env () in

  Alcotest.match_raises "unknown free variable should raise"
    (fun exn ->
      match exn with
...
let test_const_unknown_fails () =
  ...
  Alcotest.match_raises "unknown constant should raise"
    (fun exn ->
      match exn with
...
let test_empty_constants () =
  ...
  Alcotest.check' Testable.term ~msg:"Empty is of Sort 1" ~actual:empty_ty
    ~expected:(Sort 1);

  (* Empty.elim : (C : Type) -> Empty -> C *)
  let elim_ty = try_infer env.kenv lctx (Const "Empty.elim") in
  Alcotest.check' Testable.term ~msg:"Empty.elim has correct type"
    ~actual:elim_ty
...
let test_and_constants () =
  ...
  Alcotest.check' Testable.term ~msg:"And has correct type" ~actual:and_ty
  ...
  Alcotest.check' Testable.term ~msg:"And.intro has correct type"
    ~actual:intro_ty
    ~expected:
      (Forall
         ( Sort 0,
           Forall
             ( Sort 0,
               Forall
                 ( Bvar 1,
                   Forall (Bvar 1, App (App (Const "And", Bvar 3), Bvar 2)) ) )
         ));

  (* And.elim : (A : Prop) -> (B : Prop) -> (C : Prop) -> (f : A -> B -> C) -> (h : And A B) -> C *)
  let elim_ty = try_infer env.kenv lctx (Const "And.elim") in
  Alcotest.check' Testable.term ~msg:"And.elim has correct type" ~actual:elim_ty
    ~expected:
      (Forall
         ( Sort 0,
           Forall
...
let test_infer_function_type () =
  ...
  Alcotest.check' Testable.term ~msg:"const_func has correct type"
  ...
  Alcotest.check' Testable.term ~msg:"identity_func has correct type"
    ~actual:(try_infer env (Hashtbl.create 0) identity_func)
    ~expected:(Forall (Const "Point", Const "Point"));

  let generic_func = Lam (Sort 1, Lam (Bvar 0, Bvar 0)) in
  Alcotest.check' Testable.term ~msg:"generic_func has correct type"
...
let test_infer_forall () =
  ...
  Alcotest.check' Testable.term ~msg:"forall_term has correct type"
  ...
  Alcotest.check' Testable.term ~msg:"generic_func_type has correct type"
  ...
  let pred_forall =
    Forall
      ( Const "Point",
        Forall (App (Const "IsRed", Bvar 0), App (Const "IsRed", Bvar 1)) )
  in
  (* the forall statement is a Prop by impredicativity *)
  Alcotest.check' Testable.term ~msg:"pred_forall has correct type"
    ~actual:(try_infer env (Hashtbl.create 0) pred_forall)
    ~expected:(Sort 0);

  let bigger_pred_forall = Forall (Sort 67, App (Const "IsRed", Const "p")) in
  Alcotest.check' Testable.term ~msg:"bigger_pred_forall has correct type"
  ...
  (* (Point -> Prop) : Type *)
  Alcotest.check' Testable.term ~msg:"motive has correct type"
    ~actual:(try_infer env (Hashtbl.create 0) motive)
    ~expected:(Sort 1);

  (* failure cases *)
  let return_type_not_sort = Forall (Const "Point", Const "p") in
  Alcotest.match_raises "return type not sort should raise"
    (fun exn ->
      match exn with
      | TypeError { err_kind = ForallSortError _; _ } -> true
      | _ -> false)
    (fun () -> ignore (try_infer env (Hashtbl.create 0) return_type_not_sort));

  let domain_type_not_sort =
    Forall (Const "p", App (Const "IsRed", Const "p"))
  in
  Alcotest.match_raises "domain type not sort should raise"
    (fun exn ->
      match exn with
      | TypeError { err_kind = ForallSortError _; _ } -> true
      | _ -> false)
  ...
  Alcotest.match_raises "domain and return type not sort should raise"
    (fun exn ->
      match exn with
      | TypeError { err_kind = ForallSortError _; _ } -> true
      | _ -> false)
...
let test_infer_function_application () =
  ...
  Alcotest.check' Testable.term ~msg:"const_func_app has correct type"
  ...
  Alcotest.check' Testable.term ~msg:"identity_func_app has correct type"
...
is equivalent to `fun (x: Point) -> x`, so we'd expect a type of `Point => Point` *)
  let generic_func_app =
    App (Lam (Sort 1, Lam (Bvar 0, Bvar 0)), Const "Point")
  in
  Alcotest.check' Testable.term ~msg:"generic_func_app has correct type"
    ~actual:(try_infer env local_ctx generic_func_app)
    ~expected:(Forall (Const "Point", Const "Point"))

let test_subst_bvar () =
  Alcotest.check' Testable.term ~msg:"no bvars"
    ~actual:(subst_bvar (Const "Point") 0 (Const "p"))
    ~expected:(Const "Point");

  Alcotest.check' Testable.term ~msg:"correct index"
    ~actual:(subst_bvar (Bvar 0) 0 (Const "p"))
    ~expected:(Const "p");

  Alcotest.check' Testable.term ~msg:"wrong index"
    ~actual:(subst_bvar (Bvar 0) 1 (Const "l"))
    ~expected:(Bvar 0);

  Alcotest.check' Testable.term ~msg:"substitute with bvar"
  ...
  Alcotest.check' Testable.term ~msg:"lambda argument but not inner bvar"
    ~actual:(subst_bvar (Lam (Bvar 0, Bvar 0)) 0 (Const "Point"))
    ~expected:(Lam (Const "Point", Bvar 0));

  Alcotest.check' Testable.term ~msg:"lambda argument and inner bvar"
    ~actual:(subst_bvar (Lam (Bvar 0, Bvar 1)) 0 (Const "Point"))
    ~expected:(Lam (Const "Point", Const "Point"));

  Alcotest.check' Testable.term ~msg:"lambda inner bvar unchanged"
  ...
  Alcotest.check' Testable.term ~msg:"forall argument but not inner bvar"
    ~actual:(subst_bvar (Forall (Bvar 0, Bvar 0)) 0 (Sort 5))
    ~expected:(Forall (Sort 5, Bvar 0));

  Alcotest.check' Testable.term ~msg:"forall argument and inner bvar"
    ~actual:(subst_bvar (Forall (Bvar 0, Bvar 1)) 0 (Sort 5))
    ~expected:(Forall (Sort 5, Sort 5));

  Alcotest.check' Testable.term ~msg:"forall inner bvar unchanged"
    ~actual:(subst_bvar (Forall (Bvar 0, Bvar 1)) 1 (Sort 5))
    ~expected:(Forall (Bvar 0, Bvar 1))

let test_rebind_bvar () =
  Alcotest.check' Testable.term ~msg:"no bvars or fvars"
    ~actual:(rebind_bvar (Const "Point") 0 "p")
    ~expected:(Const "Point");

  Alcotest.check' Testable.term ~msg:"correct fvar name"
    ~actual:(rebind_bvar (Fvar "p") 0 "p")
    ~expected:(Bvar 0);

  Alcotest.check' Testable.term ~msg:"correct fvar name, index 1"
    ~actual:(rebind_bvar (Fvar "p") 1 "p")
    ~expected:(Bvar 1);

  Alcotest.check' Testable.term ~msg:"incorrect fvar name"
    ~actual:(rebind_bvar (Fvar "p") 0 "l")
    ~expected:(Fvar "p");

  Alcotest.check' Testable.term ~msg:"bvar unchanged"
    ~actual:(rebind_bvar (Bvar 0) 0 "p")
    ~expected:(Bvar 0);

  Alcotest.check' Testable.term ~msg:"bvar 5 unchanged"
    ~actual:(rebind_bvar (Bvar 5) 0 "p")
    ~expected:(Bvar 5);

  Alcotest.check' Testable.term ~msg:"bvar unchanged, index 5"
    ~actual:(rebind_bvar (Bvar 0) 5 "p")
    ~expected:(Bvar 0);

  Alcotest.check' Testable.term ~msg:"bvar 5 unchanged, index 5"
    ~actual:(rebind_bvar (Bvar 5) 5 "p")
    ~expected:(Bvar 5);

  Alcotest.check' Testable.term ~msg:"lambda argument"
    ~actual:(rebind_bvar (Lam (Fvar "Point", Bvar 0)) 0 "Point")
    ~expected:(Lam (Bvar 0, Bvar 0));

  Alcotest.check' Testable.term ~msg:"lambda argument and matching inner fvar"
    ~actual:(rebind_bvar (Lam (Fvar "Point", Fvar "Point")) 0 "Point")
    ~expected:(Lam (Bvar 0, Bvar 1));

  Alcotest.check' Testable.term ~msg:"lambda argument and not inner fvar"
    ~actual:(rebind_bvar (Lam (Fvar "Point", Fvar "Line")) 0 "Point")
    ~expected:(Lam (Bvar 0, Fvar "Line"));

  Alcotest.check' Testable.term ~msg:"bvar unchanged in lambda"
...
let test_eq_symm () =
  ...
  Alcotest.check' Testable.term ~msg:"application_multiple_arguments works"
    ~actual:
      (application_multiple_arguments (Const "f")
              ...
              ( Bvar 1,
                ...
                Forall
                  ( eq (Bvar 2) (Bvar 1) (Bvar 0),
                    ...
                    (* Eq b a *) ) ) ) )
  in
  Alcotest.check' Testable.term ~msg:"eq_symm_type has correct type"
  ...
  Alcotest.check'
    ...
    ~msg:"eq_symm_term has correct type" ~actual:inferred_type
...
let test_len_app () =
  ...
  (* Lt Zero Zero : Prop *)
  Alcotest.check' Testable.term ~msg:"Lt Zero Zero is of type Prop"
    ~actual:
  ...
  Alcotest.match_raises "Lt applied to a Point should raise"
...
let test_kernel_reduce () =
  ...
  let term =
    App
      ( App (Lam (Const "Point", Lam (Const "Point", Bvar 0)), Const "p"),
        Const "l" )
  in
  Alcotest.check' Testable.term ~msg:"application to nested lambdas"
    ~actual:(reduce env lctx term) ~expected:(Const "l");

  (* fun (x: (fun a => Point) p) => x is well-typed and should not fail inference *)
  (* (reduces to fun (x: Point) => x) *)
  let term =
    Lam (App (Lam (Const "Point", Const "Point"), Const "p"), Bvar 0)
  in
  Alcotest.check'
  ...
  Alcotest.check'
    ...
    ~actual:(inferType env lctx term) ~expected:(Sort 1);
...
let suite =
  ...
  ( "nicegeo",
    [
      ...
      test_case "Unknown free variable should raise" `Quick
      ...
      test_case "Infer function application" `Quick
