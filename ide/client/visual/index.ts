...
const MODIFIERS: ToolButtonDefinition[] = [{ label: "Distinct", icon: "≠" }];

const buildToolButton = ({ label, icon }: ToolButtonDefinition): HTMLElement => {
...
const buildToolGroup = (buttons: ToolButtonDefinition[]): HTMLElement => {
  const group = document.createElement("div");
  group.className = "group";
  group.append(...buttons.map(buildToolButton));
  return group;
};
...
const buildTitledSection = (
  ...
  const group = buildToolGroup(buttons);
...
const renderToolButtons = (): void => {
  ...
  const toolsGroup = buildTitledSection("Tools", TOOLS, "toolSection");
  const modifiersGroup = buildTitledSection("Modifiers", MODIFIERS, "modifierSection");
