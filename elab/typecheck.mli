...
(** [hole_to_meta ctx stack tm] returns tm unchanged except for replacing holes with metavariable spines.
...
(** [infertype ctx tm] returns the inferred type of term [tm] in context [ctx], filling in metavariables. *)
val infertype : Types.ctx -> term -> term

(** [replace_metas ctx tm] replaces all holes in [tm] with their solutions in context [ctx]. *)
val replace_metas : Types.ctx -> term -> term

(** [check_is_type ctx tm] checks that [tm] is a type in context [ctx], filling in metavariables. *)
val check_is_type : Types.ctx -> term -> unit

(** [checktype ctx tm ty] checks that [tm] has type [ty] in context [ctx], filling in metavariables. *)
