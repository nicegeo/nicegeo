...
let parse_term (s : string) : Term.term =
  ...
  let parse = MenhirLib.Convert.Simplified.traditional2revised Parser.single_term in
  parse (Sedlexing.with_tokenizer Lexer.token lexbuf)
...
let parse_statements (filename : string) : Statement.statement list =
  ...
  let decls =
    try parse (Sedlexing.with_tokenizer Lexer.token lexbuf)
    with exn ->
        ...
        (Error.ElabError
           {
             ...
             error_type =
               Error.ParseError { input = Sedlexing.Utf8.lexeme lexbuf; error_msg = msg };
           })
  in
  decls
