...
let rec valid_pattern (e : ctx) (graph : rw_graph) (m : int) (tm : term) (ctx : int list)
    : bool =
  match tm.inner with
  | Bvar bid ->
  ...
  | Fun (_, bid, ty_arg, body) ->
      valid_pattern e graph m ty_arg ctx && valid_pattern e graph m body (bid :: ctx)
  | Arrow (_, bid, ty_arg, ty_ret) ->
      valid_pattern e graph m ty_arg ctx && valid_pattern e graph m ty_ret (bid :: ctx)
...
(** recursively replaces Bvar pat with Bvar replacement in metavar solutions that appear
    in tm *)
