...
and infertype ?(depth = 0) (e : ctx) (tm : term) : term =
  ...
  let res =
    ...
    | Arrow (arg, bid, ty_arg, ty_ret) ->
        let ty_arg_ty = whnf e (infertype ~depth:(depth + 1) e ty_arg) in
        let arg_sort =
        ...
        in
        Hashtbl.add e.lctx bid (arg, ty_arg);
        let ty_ret_ty = whnf e (infertype ~depth:(depth + 1) e ty_ret) in
        let ret_sort =
    ...
    | App (f, arg) -> (
        let f_type = whnf e (infertype ~depth:(depth + 1) e f) in
