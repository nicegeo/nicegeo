...
(*
  ...
  Modes:
  - `nicegeo <file.ncg>`: parse + elaborate + kernel-check the file, printing diagnostics.
  - `nicegeo --proofstate-at [--json] <file.ncg> <line> <col>`: proof state snapshot (text, or JSON with --json).
*)
...
let () =
  ...
  | [ _; filename ] -> run_check_mode filename
  | _ :: "--proofstate-at" :: tail -> (
      match parse_proofstate_argv tail with
      | None ->
          Printf.eprintf
